...
assert_body_contains "HAL root advertises core namespace"    \
  "$BACKEND_URL/server/api"                                  '"core"'
...
assert_body_contains "Authn status — not authenticated"      \
  "$BACKEND_URL/server/api/authn/status"                     '"authenticated":false'

assert_status        "DSpace info/status returns 200"        \
  "$BACKEND_URL/server/api/info/status"                      200

assert_body_contains "DSpace info/status — dspaceVersion"    \
  "$BACKEND_URL/server/api/info/status"                      '"dspaceVersion"'
...
assert_status        "Frontend /home returns 200"            \
  "$FRONTEND_URL/home"                                       200

assert_body_contains "Frontend /home — contains app-root"   \
  "$FRONTEND_URL/home"                                       "app-root"

assert_body_contains "Frontend /home — contains DSpace title" \
  "$FRONTEND_URL/home"                                       "DSpace"

assert_body_not_contains "Frontend /home — no Angular error boundary" \
  "$FRONTEND_URL/home"                                       "ng-error"
